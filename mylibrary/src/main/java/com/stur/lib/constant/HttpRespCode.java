...
/**
 * Created by Sturmegezhutz on 2018/3/7.
...
public class HttpRespCode {
     ...
     */
    public static final int HTTP_RESP_100 = 100;
     ...
     */
    public static final int HTTP_RESP_101 = 101;

    /**
     * Processing
     * 由WebDAV（RFC 2518）扩展的状态码，代表处理将被继续执行。
     */
    public static final int HTTP_RESP_102 = 102;
     ...
     */
    public static final int HTTP_RESP_203 = 203;
     ...
     */
    public static final int HTTP_RESP_205 = 205;
     ...
     * 这避免了缓存的实体内容和更新了的实体头信息之间的不一致。否则，本响应就应当包含所有本应该返回200响应中应当返回的所有实体头部域。
     * 假如 ETag 或 Last-Modified 头部不能精确匹配的话，则客户端缓存应禁止将206响应返回的内容与之前任何缓存过的内容组合在一起。
     */
    public static final int HTTP_RESP_206 = 206;
     ...
     */
    public static final int HTTP_RESP_207 = 207;
     ...
     * 当且仅当后续的请求所使用的方法是 GET 或者 HEAD 时，
     * 用户浏览器才可以在没有用户介入的情况下自动提交所需要的后续请求。
     * 客户端应当自动监测无限循环重定向（例如：A->A，或者A->B->C->A），
     * 因为这会导致服务器和客户端大量不必要的资源消耗。
     ...
     */
    public static final int HTTP_RESP_301 = 301;
     ...
     * 注意：虽然RFC 1945和RFC 2068规范不允许客户端在重定向时改变请求的方法，
     * 但是很多现存的浏览器将302响应视作为303响应，并且使用 GET 方式访问在 Location 中规定的 URI，而无视原先请求的方法。
     * 状态码303和307被添加了进来，用以明确服务器期待客户端进行何种反应。
     */
    public static final int HTTP_RESP_302 = 302;
     ...
     * 注意：许多 HTTP/1.1 版以前的浏览器不能正确理解303状态。
     * 如果需要考虑与这些浏览器之间的互动，302状态码应该可以胜任，
     * 因为大多数的浏览器处理302响应时的方式恰恰就是上述规范要求客户端处理303响应时应当做的。
     */
    public static final int HTTP_RESP_303 = 303;
     ...
     * * Date，除非这个服务器没有时钟。
     *   假如没有时钟的服务器也遵守这些规则，
     *   那么代理服务器以及客户端可以自行将 Date 字段添加到接收到的响应头中去（正如RFC 2068中规定的一样），
     *   缓存机制将会正常工作。
     * * ETag 和/或 Content-Location，假如同样的请求本应返回200响应。
     * * Expires, Cache-Control，和/或Vary，假如其值可能与之前相同变量的其他响应对应的值不同的话。
     * 假如本响应请求使用了强缓存验证，那么本次响应不应该包含其他实体头；
     * 否则（例如，某个带条件的 GET 请求使用了弱缓存验证），
     * 本次响应禁止包含其他实体头；这避免了缓存了的实体内容和更新了的实体头信息之间的不一致。
     * 假如某个304响应指明了当前某个实体没有缓存，那么缓存系统必须忽视这个响应，并且重复发送不包含限制条件的请求。
     * 假如接收到一个要求更新某个缓存条目的304响应，那么缓存系统必须更新整个条目以反映所有在响应中被更新的字段的值。
     */
    public static final int HTTP_RESP_304 = 304;

    /**
     * Use Proxy
     * 被请求的资源必须通过指定的代理才能被访问。
     * Location 域中将给出指定的代理所在的 URI 信息，
     * 接收者需要重复发送一个单独的请求，通过这个代理才能访问相应资源。
     * 只有原始服务器才能建立305响应。
     * 注意：RFC 2068中没有明确305响应是为了重定向一个单独的请求，
     * 而且只能被原始服务器建立。忽视这些限制可能导致严重的安全后果。
     */
    public static final int HTTP_RESP_305 = 305;

    /**
     * Switch Proxy
     * 在最新版的规范中，306状态码已经不再被使用。
     */
    public static final int HTTP_RESP_306 = 306;

    /**
     * Temporary Redirect
     * 请求的资源临时从不同的URI 响应请求。
     * 新的临时性的URI 应当在响应的 Location 域中返回。除非这是一个HEAD 请求，
     * 否则响应的实体中应当包含指向新的URI 的超链接及简短说明。
     * 因为部分浏览器不能识别307响应，因此需要添加上述必要信息以便用户能够理解并向新的 URI 发出访问请求。
     * 如果这不是一个GET 或者 HEAD 请求，那么浏览器禁止自动进行重定向，
     * 除非得到用户的确认，因为请求的条件可能因此发生变化。
     */
    public static final int HTTP_RESP_307 = 307;
     ...
     * 以及这是临时的还是永久性的状况。这些状态码适用于任何请求方法。
     * 浏览器应当向用户显示任何包含在此类错误响应中的实体内容。
     * 如果错误发生时客户端正在传送数据，
     * 那么使用TCP的服务器实现应当仔细确保在关闭客户端与服务器之间的连接之前，
     * 客户端已经收到了包含错误信息的数据包。
     * 如果客户端在收到错误信息后继续向服务器发送数据，
     * 服务器的TCP栈将向客户端发送一个重置数据包，以清除该客户端所有还未识别的输入缓冲，
     ...
     * 该状态码是为了将来可能的需求而预留的。
     */
    public static final int HTTP_RESP_402 = 402;
     ...
     */
    public static final int HTTP_RESP_403 = 403;
     ...
     */
    public static final int HTTP_RESP_404 = 404;
     ...
     */
    public static final int HTTP_RESP_405 = 405;
     ...
     */
    public static final int HTTP_RESP_406 = 406;
     ...
     */
    public static final int HTTP_RESP_407 = 407;
     ...
     */
    public static final int HTTP_RESP_408 = 408;
     ...
     * 冲突通常发生于对 PUT 请求的处理中。
     * 例如，在采用版本检查的环境下，某次 PUT 提交的对特定资源的修改请求所附带的版本信息与之前的某个（第三方）请求向冲突，
     * 那么此时服务器就应该返回一个409错误，告知用户请求无法完成。
     * 此时，响应实体中很可能会包含两个冲突版本之间的差异比较，以便用户重新提交归并以后的新版本。
     */
    public static final int HTTP_RESP_409 = 409;
     ...
     * 除非额外说明，否则这个响应是可缓存的。
     * 响应的目的主要是帮助网站管理员维护网站，通知用户该资源已经不再可用，
     * 并且服务器拥有者希望所有指向这个资源的远端连接也被删除。
     * 这类事件在限时、增值服务中很普遍。
     * 同样，410响应也被用于通知客户端在当前服务器站点上，原本属于某个个人的资源已经不再可用。
     * 当然，是否需要把所有永久不可用的资源标记为'410 Gone'，
     * 以及是否需要保持此标记多长时间，完全取决于服务器拥有者。
     */
    public static final int HTTP_RESP_410 = 410;
     ...
     */
    public static final int HTTP_RESP_411 = 411;
     ...
     */
    public static final int HTTP_RESP_412 = 412;

    /**
     * Request Entity Too Large
     * 服务器拒绝处理当前请求，因为该请求提交的实体数据大小超过了服务器愿意或者能够处理的范围。
     * 此种情况下，服务器可以关闭连接以免客户端继续发送此请求。
     * 如果这个状况是临时的，服务器应当返回一个 Retry-After 的响应头，
     * 以告知客户端可以在多少时间以后重新尝试。
     */
    public static final int HTTP_RESP_413 = 413;
     ...
     *   这类服务器使用固定长度的缓冲读取或操作请求的 URI，
     *   当 GET 后的参数超过某个数值后，可能会产生缓冲区溢出，导致任意代码被执行。
     *   没有此类漏洞的服务器，应当返回414状态码。
     */
    public static final int HTTP_RESP_414 = 414;

    /**
     * Unsupported Media Type
     * 对于当前请求的方法和所请求的资源，请求中提交的实体并不是服务器中所支持的格式，因此请求被拒绝。
     */
    public static final int HTTP_RESP_415 = 415;
     ...
     */
    public static final int HTTP_RESP_416 = 416;
     ...
     */
    public static final int HTTP_RESP_417 = 417;
     ...
     */
    public static final int HTTP_RESP_421 = 421;

    /**
     * Unprocessable Entity
     * 请求格式正确，但是由于含有语义错误，无法响应。（RFC 4918 WebDAV）
     */
    public static final int HTTP_RESP_422 = 422;

    /**
     * Locked
     * 当前资源被锁定。（RFC 4918 WebDAV）
     */
    public static final int HTTP_RESP_423 = 423;

    /**
     * Failed Dependency
     * 由于之前的某个请求发生的错误，导致当前请求失败，例如 PROPPATCH。（RFC 4918 WebDAV）
     */
    public static final int HTTP_RESP_424 = 424;

    /**
     * Unordered Collection
     * 在WebDav Advanced Collections 草案中定义，但是未出现在《WebDAV 顺序集协议》（RFC 3658）中。
     */
    public static final int HTTP_RESP_425 = 425;

    /**
     * Upgrade Required
     * 客户端应当切换到TLS/1.0。（RFC 2817）
     */
    public static final int HTTP_RESP_426 = 426;

    /**
     * Retry With
     * 由微软扩展，代表请求应当在执行完适当的操作后进行重试。
     */
    public static final int HTTP_RESP_449 = 449;

    /**
     * Unavailable For Legal Reasons
     * 该请求因法律原因不可用。（RFC 7725）
     */
    public static final int HTTP_RESP_451 = 451;
     ...
     */
    public static final int HTTP_RESP_500 = 500;
     ...
     */
    public static final int HTTP_RESP_501 = 501;

    /**
     * Bad Gateway
     * 作为网关或者代理工作的服务器尝试执行请求时，从上游服务器接收到无效的响应。
     */
    public static final int HTTP_RESP_502 = 502;
     ...
     * 某些服务器只不过是希望拒绝客户端的连接。
     */
    public static final int HTTP_RESP_503 = 503;
     ...
     */
    public static final int HTTP_RESP_504 = 504;
     ...
     */
    public static final int HTTP_RESP_505 = 505;

    /**
     * Variant Also Negotiates
     * 由《透明内容协商协议》（RFC 2295）扩展，代表服务器存在内部配置错误：
     * 被请求的协商变元资源被配置为在透明内容协商中使用自己，
     * 因此在一个协商处理中不是一个合适的重点。
     */
    public static final int HTTP_RESP_506 = 506;

    /**
     * Insufficient Storage
     * 服务器无法存储完成请求所必须的内容。这个状况被认为是临时的。WebDAV (RFC 4918)
     */
    public static final int HTTP_RESP_507 = 507;

    /**
     * Bandwidth Limit Exceeded
     * 服务器达到带宽限制。这不是一个官方的状态码，但是仍被广泛使用。
     */
    public static final int HTTP_RESP_509 = 509;

    /**
     * Not Extended
     * 获取资源所需要的策略并没有被满足。（RFC 2774）
     */
    public static final int HTTP_RESP_510 = 510;

    /**
     * Unparseable Response Headers
     * 源站没有返回响应头部，只返回实体内容
     */
    public static final int HTTP_RESP_600 = 600;
}
